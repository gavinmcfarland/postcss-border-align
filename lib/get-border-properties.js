...
colorRegex = colorRegex.source + "|" + colors;
colorRegex = new RegExp(colorRegex, 'gi');
    ...
    rule.walkDecls(/^border/, decl => {
      ...
      for (let side in border) {
        for (let b = 0; b < schema.length; b++) {
          ...
          let bordersSidesProps2 =
            "border" + "-" + schema[b].name;
          ...
          // Look through individual properties eg border-top-width
          ...
          else if (decl.prop === bordersSidesProps2) {
            if (decl.prop === bordersSidesProps2) {
              border[side][schema[b].name] = decl.value;
            } else {
              border[side][schema[b].name] = decl.value;
            }
